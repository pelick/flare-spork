...
public abstract class Rule {
        ...
        while(iter.hasNext()) {
            ...
            if (match(op, leaves.get(0))) {
                ...
                if (leaves.size()>1) {
                    ...
                    preds = plan.getPredecessors(op); // 该op的前辈们
                    ...
                    for(Operator s: preds) { // 对每个前辈
                        matched = true;
                        List<Operator> siblings = null; // 得到op的所有兄弟(包括了自己)
                        if (s != null) {
                            siblings = plan.getSuccessors(s); // 前辈的后辈们，即op的兄弟们
                        }else{
                            // for a root, we get its siblings by getting all roots
                            siblings = plan.getSources(); // 如果op是root，那么op的兄弟们就是整个plan的source们
                        }

                        int index = siblings.indexOf(op); // op在兄弟集合里的index
                        ...
                        for(int j=1; j<leaves.size(); j++) {
                            if (!match(siblings.get(index+j), leaves.get(j))) { // 每个leave和op的某个兄弟是否也match
                                matched = false;
                                break;
                            }
                        }     
                        
                        if (matched) { // 兄弟们也都match，则把这些节点都添加到planOps里
                            for(int j=1; j<leaves.size(); j++) {
                ...
                if (match.check(planOps)) { // 通过这层check之后，才会最终添加到match集合里
    ...
    private class PatternMatchOperatorPlan extends OperatorSubPlan {
        ...
        protected boolean check(List<Operator> planOps) throws FrontendException {
            List<Operator> patternOps = pattern.getSinks(); // 得到叶子节点集
            ...
            for(int i=0; i<planOps.size(); i++) {
                ...
                if (!check(planOps.get(i), patternOps.get(i), s)) { // 针对每一对planOps和patternOps进行check检查，需要全部通过
                    return false;
                }
        ...
        private boolean check(Operator planOp, Operator patternOp, Deque<Operator> opers) throws FrontendException {
            if (!match(planOp, patternOp)) { // 确保两者没有子集关系
                return false;
            }
